...
${globs.map(g => ` * \`${g}\``).join("\n")}`,
		...
		const groupByFile = _.map(
			...
			(v, k) => `${slack.bold(
				slack.url(
					`https://github.com/${repo.owner}/${repo.name}/blob/${push.branch}/${k}`,
					k,
				),
			)}:
\`\`\`
...
${groupByFile.join("\n")}`,
		...
		const users = _.uniq(
			[
				push.after.author?.person?.chatId?.screenName,
				push.after.committer?.person?.chatId?.screenName,
			].filter(u => !!u),
		);
		if (users.length > 0) {
...
${globs.map(g => ` * \`${g}\``).join("\n")}`,
	...
	if (result.excluded?.length > 0) {
		await ctx.audit
			.log(`Scanning repository returned the following excluded ${
